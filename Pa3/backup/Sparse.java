...
public class Sparse{
	public static void main(String[] args) throws IOException, FileNotFoundException{
		...
		String blank = in.nextLine();											// place holder to skip over blank line seperating data
		while (in.hasNextLine() && lineNum < Ainput){							// scan lines to enter the entries into the matrix
			String[] entry = in.nextLine().split("\\s+");						// read line and parse it
			A.changeEntry(Integer.parseInt(entry[0]), Integer.parseInt(entry[1]), Double.parseDouble(entry[2]));	// take parsed line and add entry to A
			lineNum++;															// increment the line counter
		}

		blank = in.nextLine();													// repeat above while() but for matrix B
